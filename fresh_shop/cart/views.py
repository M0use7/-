...
def f_place(request):
    ...
    if request.method == 'GET':
        ...
        if user_id:
            ...
            carts = ShoppingCart.objects.filter(user_id=user_id)
            cart_data = {}
            cart_data['goods_price'] = [(cart.good_id,
...
def change_goods_num(request):
    if request.method == 'POST':
        # 修改购物车中商品的个数
        # 1.先判断用户登录与否，如果用户没有登录，则修改session中的个数
        # 2.如果用户登录，需要判断当前修改的商品是否在session中，如果存在，则修改session，如果不存在则修改购物车的表
        ...
        if session_goods:
            for goods in session_goods:
                if goods_id == goods[0]:
                    ...
                    goods[1] = goods_num
                    goods[2] = is_select
                request.seesion['goods'] = session_goods

            # 如果用户登录了，则需要在修改购物车中数据，因为session中的商品有可能并不在购物车表中
            if user_id:
                # 修改购物车中商品个数
                ShoppingCart.objects.filter(user_id=user_id, goods_id=goods_id).updata(nums=goods_num,
                                                                                       is_select=is_select)
